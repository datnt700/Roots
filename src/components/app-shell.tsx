...
const SidebarLink = styled(Link)<{ $active: boolean }>(({ $active }) => ({
  display: 'flex',
  alignItems: 'center',
  gap: theme.spacing[3],
...
const MainContent = styled.div({
  ...
  flexDirection: 'column',
  minHeight: '100dvh',
  paddingBottom: BOTTOM_NAV_HEIGHT, // space for bottom nav on mobile
  '@media (min-width: 1024px)': {
...
const BottomNavItem = styled(Link)<{ $active: boolean }>(({ $active }) => ({
  flex: 1,
  display: 'flex',
  flexDirection: 'column',
  alignItems: 'center',
  ...
  const sidebarNav = (
    <>
      ...
      <SidebarBottom>
        ...
        <SidebarAction style={{ color: 'var(--destructive)' }}>
          <LogOut />
    ...
    <Shell>
      ...
      <MainContent>
        ...
        <TopHeader>
          ...
          <HeaderActions>
            <IconButton aria-label="Add parent">
              <Plus />
            </IconButton>
            <IconButton
